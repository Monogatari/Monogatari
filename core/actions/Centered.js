...
export class Centered extends Action {
		...
		if (!Monogatari.global ('finishedTyping') && Monogatari.global ('textObject') !== null) {
			...
			if (typeof parent !== 'undefined') {
				...
				if (element == 'centered') {
					...
					Monogatari.element ().find (`[data-ui="centered"] div`).html (str);
					Monogatari.global ('finishedTyping', true);
					return Promise.reject ('TypeWriter effect has not finished. Skipping it.');
				}
			}
		} else if (Monogatari.global ('finishedTyping') && Monogatari.element ().find (`[data-ui="centered"]`).isVisible ()) {
			Monogatari.element ().find (`[data-ui="centered"]`).remove ();
			Monogatari.element ().find (`[data-ui="text"]`).show ();
		}
		return Promise.resolve (Monogatari.global ('finishedTyping'));
	}

	static canRevert () {
		if (Monogatari.element ().find (`[data-ui="centered"]`).isVisible ()) {
			Monogatari.element ().find (`[data-ui="centered"]`).remove ();
			Monogatari.global ('finishedTyping', true);
			Monogatari.global ('textObject').destroy ();
	...
	apply () {
		Monogatari.element ().find (`[data-ui="text"]`).hide ();
		Monogatari.element ().find (`[data-screen="game"]`).append ('<div class="middle align-center" data-ui="centered"><div></div></div>');
		if (this.animate) {
			...
			Monogatari.element ().find (`[data-ui="centered"] div`).html (this.dialog);
		}

...
class SaveScreen extends Component {
		...
		Monogatari.registerListener ('overwrite-slot', {
			callback: () => {
				...
				if (customName !== '') {
					...
					Monogatari.dismissAlertDialog ('slot-overwrite');
				}
			}
		});
	...
	static init () {
		...
		if (!window.localStorage) {
			this.content ('slots').html (`<p>${Monogatari.string ('LocalStorageWarning')}</p>`);
		}
...
SaveScreen._html = `
	<section data-component="save-screen" data-screen="save">
		...
		<div data-ui="slots" data-content="slots" class="row row--spaced padded"></div>
	</section>
`;
